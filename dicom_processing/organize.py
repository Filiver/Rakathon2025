...
def generate_report(sample_dir, output_dir):
    ...
    for filename in os.listdir(sample_dir):
        ...
        try:
            ...
            if modality == "RTSTRUCT" and filename.upper().startswith("RS"):
                            ...
                            for contour in roi_contour.ContourSequence:
                                if "ContourImageSequence" in contour:
                                    for contour_image in contour.ContourImageSequence:
                                        ref_uid = contour_image.get("ReferencedSOPInstanceUID")
                                        if ref_uid:
                                            referenced_uids_in_rs.add(ref_uid)
                                            all_referenced_ct_uids.add(ref_uid)

                rs_files_info.append({"filename": filename, "date": file_date, "ds": ds, "referenced_uids": referenced_uids_in_rs})
                ...
                ct_files_info[sop_instance_uid] = {"filename": filename, "date": file_date, "frame_uid": frame_uid, "ds": ds}
    ...
    rs_files_info.sort(key=lambda x: x["date"] if x["date"] else datetime.max.date())
    ...
    if not rs_files_info:
        ...
        for rs_info in rs_files_info:
            rs_date_str = rs_info["date"].strftime("%Y-%m-%d") if rs_info["date"] else "Unknown Date"
            ...
            for ref_uid in rs_info["referenced_uids"]:
                if ref_uid in ct_files_info:
                    ...
                    related_cts_by_frame[ct_info["frame_uid"]].append(ct_info["filename"])
                    processed_ct_for_rs.add(ref_uid)  # Mark this CT as processed

            if not related_cts_by_frame:
                report_lines.append("  -> No related CT files found or CT files missing FrameOfReferenceUID.")
            else:
                report_lines.append("  -> Related CT Scans (Grouped by Frame of Reference UID):")
                ...
                for frame_uid, ct_filenames in sorted(related_cts_by_frame.items()):
                    ...
                    report_lines.append(f"       Files (Count: {len(ct_filenames)}): {', '.join(ct_filenames)}")

    report_lines.append("\n" + "=" * 30)
    report_lines.append("Unrelated CT Scans (Grouped by Date and Frame UID)")
    report_lines.append("-" * 30)

    unrelated_ct_groups = defaultdict(lambda: defaultdict(list))  # {date: {frame_uid: [filenames]}}
    ...
    if not unrelated_ct_found:
        ...
        sorted_dates = sorted(unrelated_ct_groups.keys(), key=lambda d: d if d else datetime.max.date())
        for date in sorted_dates:
            ...
            for frame_uid, filenames in sorted(frame_groups.items()):
                ...
                filenames.sort()
                report_lines.append(f"  -> Frame UID: {frame_uid}")
                report_lines.append(f"     Files (Count: {len(filenames)}): {', '.join(filenames)}")
...
REPORTS_OUTPUT_DIRECTORY = "data/radioprotect/Rackaton Data"
# -------------------
