...
def add_frame_of_reference(output, frame_of_reference, modality, file):
    if frame_of_reference not in output:
        output[frame_of_reference] = {}
    if modality not in output[frame_of_reference]:
        ...
        if os.path.isdir(item_path):
            ...
            try:
                ...
                if ds.Modality == "RTSTRUCT":
                    for rfors in ds.ReferencedFrameOfReferenceSequence:
                        frame_of_reference = rfors.FrameOfReferenceUID
                        add_frame_of_reference(output, frame_of_reference, "RTSTRUCT", item)
                else:
                    add_frame_of_reference(output, ds.FrameOfReferenceUID, ds.Modality, item)
        ...
        for sample in output:
            ...
            for identifier in output[sample]:
                ...
                for modality in output[sample][identifier]:
                    counts[sample][identifier][modality] = len(output[sample][identifier][modality])
