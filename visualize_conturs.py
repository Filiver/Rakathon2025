...
def visualize_all_contours_from_dict2(contours_dict, scan_data, spacing, origin):
    ...
    if scan_data.ndim != 3:
        print(f"Error: scan_data must be 3D (D, H, W), got shape {scan_data.shape}")
        return
    if origin_np.shape != (3,) or spacing_np.shape != (3,):
        print(f"Error: Origin shape {origin_np.shape} or Spacing shape {spacing_np.shape} is not (3,).")
        return
    ...
    plt.figure(figsize=(15, 5 * ((num_slices + 4) // 5))) # Adjust layout as needed

    slice_has_contour = {i: False for i in range(num_slices)}
    contours_per_slice = {i: {} for i in range(num_slices)} # slice_idx -> {roi_name: np.array(N, 2)}
    ...
    for roi_name, points_tensor in contour_sub_dict.items():
        if not isinstance(points_tensor, torch.Tensor):
            print(f"Warning: Skipping ROI '{roi_name}' - value is not a PyTorch tensor.")
            continue
        ...
        try:
            ...
            print(f"Warning: Skipping ROI '{roi_name}' - error converting tensor to NumPy: {e}")
            continue

        # **** Critical Check for dimensionality and shape ****
        if points_np.ndim != 2 or points_np.shape[1] != 3:
            print(f"Warning: Skipping ROI '{roi_name}' - NumPy array shape is {points_np.shape}, expected (N, 3).")
            continue
        # **** End Critical Check ****
    ...
    for slice_idx in contours_per_slice:
        for roi_name in contours_per_slice[slice_idx]:
            if contours_per_slice[slice_idx][roi_name]: # Check if list is not empty
                 contours_per_slice[slice_idx][roi_name] = np.array(contours_per_slice[slice_idx][roi_name])
            else:
                 contours_per_slice[slice_idx][roi_name] = np.empty((0,2)) # Use empty array if no points
    ...
    for slice_idx in range(num_slices):
        ...
        ax = plt.subplot(5, (num_slices + 4) // 5, plot_index) # Adjust grid as needed
        ax.imshow(scan_data[slice_idx], cmap='gray', aspect='equal') # Display the Z slice (axial view)
        ...
        for roi_name, hw_points in rois_on_slice.items():
             ...
             if hw_points.ndim == 2 and hw_points.shape[0] > 0 and hw_points.shape[1] == 2:
                 ...
                 ax.plot(hw_points[:, 1], hw_points[:, 0], '.', label=roi_name, markersize=2) # Plot w vs h
                 slice_had_roi = True
